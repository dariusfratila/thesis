from modules.models.lateral_inhibition import LateralInhibition
import torch
...
class TCNBlock(nn.Module):
    ...
    used in temporal sequence modeling for capturing dependencies across time steps.
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, dilation=1):
        ...
        self.conv = nn.Conv1d(in_channels, out_channels, kernel_size, padding=(
            kernel_size-1) * dilation, dilation=dilation)
        self.bnorm = nn.BatchNorm1d(out_channels)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, x):
        """
        Forward pass of the TCNBlock.
...
class MS_TCN(nn.Module):
    ...
    This model applies multiple layers of TCN blocks with varying kernel sizes to capture
    dependencies at different scales in the input sequence.

    Args:
        num_layers (int): Number of layers in the MS-TCN.
        in_channels (int): Number of input channels.
        out_channels (int): Number of output channels.
        kernel_sizes (list, optional): List of kernel sizes for each layer. Default is [3, 5, 7].
    """
    ...
    def __init__(self, num_layers, in_channels, out_channels, kernel_sizes=[3, 5, 7]):
        ...
        for i in range(num_layers):
            ...
            layers.append(TCNBlock(in_channels if i == 0 else out_channels,
                                   out_channels, kernel_size, dilation=2**i))
        self.network = nn.Sequential(*layers)

    def forward(self, x):
        """
        Forward pass of the MS-TCN.

        Args:
            x (torch.Tensor): Input tensor of shape (batch_size, channels, sequence_length).

        Returns:
            torch.Tensor: Output tensor after applying the MS-TCN.
        """
...
class LipReadModel(nn.Module):
    """
    LipReading Model for Visual Speech Recognition.

    This model uses 3D convolutions to process input video frames, a ResNet backbone for feature extraction,
    and a Multi-Scale Temporal Convolutional Network (MS-TCN) to capture temporal dependencies in the features.
    It also applies lateral inhibition to emphasize important features.

    Args:
        num_classes (int): Number of output classes (words) for classification.
    """
    ...
    def forward(self, x):
        """
        Forward pass for the LipReadModel.

        Args:
            x (torch.Tensor): Input tensor of shape (batch_size, channels, depth, height, width).

        Returns:
            torch.Tensor: The model's output, a tensor representing the predicted class probabilities.
        """
    ...
    def _initialize_weights(self):
        """
        Initializes weights for the convolutional, batch normalization, and linear layers.

        This method ensures the model starts with properly initialized parameters,
        which helps in stabilizing training and improving model performance.
        """
